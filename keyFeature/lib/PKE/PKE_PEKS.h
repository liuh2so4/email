...
typedef struct Parameter{
    ...
    mpz_t q;
    element_t g;
    void (*H1)(char*, element_t, size_t);
    void (*H2)(element_t, char*, int, element_t);
    void (*H3)(char*, element_t, size_t);
    void (*H4)(element_t, char*, element_t, char*, char*, size_t, char**);
} PARAMETER;

void KeyGen_H1_H3(char* hash, element_t element, size_t output_size) {
...
void print_H1_H3_result(char *hash_result, size_t length) {
    printf("Hash Result: ");
...
void PKE_Encrypt(PARAMETER params, element_t pk, char* w, char* plaintext, element_t c1, char** c2, char** tau, char** sigma) {
    ...
    char* binaryMessage = (char*) malloc(binaryLength + 1);;
    charToBinary(plaintext, mLength, binaryMessage);
    *c2 = (char*) malloc(binaryLength + 1);
    ...
    *tau = (char*)malloc((SHA256_DIGEST_LENGTH) * sizeof(char));
...
char* PKE_Decrypt(PARAMETER params,mpz_t sk, element_t c1, char* c2, char* tau, char* sigma) {
    element_t kappa;
    char* K = (char*)malloc((SHA256_DIGEST_LENGTH) * sizeof(char));
    element_init_G1(kappa, params.pairing);
    ...
    // printf("\n");
    char* m = (char*) malloc(binaryLength / 8);
